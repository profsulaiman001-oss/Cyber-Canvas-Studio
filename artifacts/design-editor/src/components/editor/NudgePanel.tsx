...
export default function NudgePanel({ onNudge }: NudgePanelProps) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && dispatch({ type: 'CLOSE_PANEL' })}>
      ...
      >
        <SheetHeader className="px-4 pt-2 pb-1 flex flex-row items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              disabled={stepIdx <= 0}
              className="w-6 h-6 bg-slate-900 rounded flex items-center justify-center text-xs font-bold text-neutral-400 disabled:opacity-30 border border-neutral-800 hover:border-cyan-500/40"
            >
              −
            </button>
            <span className="text-xs font-mono font-bold w-10 text-center" style={{ color: '#00F5FF' }}>
              {nudgeStep}px
            </span>
            <button
        ...
        <div
          ...
          style={{ paddingTop: '4px', paddingBottom: 'max(8px, env(safe-area-inset-bottom))' }}
        >
          <div className="grid grid-cols-3 gap-1.5" style={{ width: '136px', height: '136px' }}>
            <div />
            <button className={arrowBtn} onClick={() => onNudge('up', nudgeStep)}>▲</button>
            <div />
            <button className={arrowBtn} onClick={() => onNudge('left', nudgeStep)}>◀</button>
            <div
            ...
            <button className={arrowBtn} onClick={() => onNudge('right', nudgeStep)}>▶</button>
            <div />
            <button className={arrowBtn} onClick={() => onNudge('down', nudgeStep)}>▼</button>
            <div />
          </div>
        </div>
      </SheetContent>
